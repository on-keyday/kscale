...
#   metrics        : consts + stat structs + typed_stats.proto (before protoc)
...
#   5. proto compat: no field/enum number of any .proto may change vs HEAD (the
#                    generators number by order, so an insert silently renumbers;
#                    see script/check_proto_compat.py)
#
...
# It runs BEFORE protoc so typed_stats.pbg.go is compiled from the typed_stats.proto
# this run produced (after protoc, one run left the .pbg.go a generation behind).
PY=python3
...
echo "[compat] proto wire compatibility vs ${PROTO_COMPAT_BASE:-HEAD}"
python3 script/check_proto_compat.py
