...
// LoadGeoLocation builds a GeoLocationInfo from already-fetched mmdb bytes,
// with no network access — the load half of FetchGeoLocation, for callers that
// obtain the databases through a separate channel (e.g. a distributed file).
// Both ASN and City databases are required.
func LoadGeoLocation(asnMMDB, cityMMDB []byte) (*GeoLocationInfo, error) {
	dbs := &GeoLocationInfo{}
	if len(asnMMDB) > 0 {
		r, err := geoip2.OpenBytes(asnMMDB)
		if err != nil {
			return nil, fmt.Errorf("failed to open ASN mmdb: %w", err)
		}
		dbs.asn = r
	}
	if len(cityMMDB) > 0 {
		r, err := geoip2.OpenBytes(cityMMDB)
		if err != nil {
			return nil, fmt.Errorf("failed to open City mmdb: %w", err)
		}
		dbs.city = r
	}
	if dbs.asn == nil || dbs.city == nil {
		return nil, fmt.Errorf("both ASN and City databases are required (asn=%v city=%v)", dbs.asn != nil, dbs.city != nil)
	}
	// Same reader-pointer cleanup as FetchGeoLocation (see the note there on why
	// the cleanup captures the readers, not dbs).
	runtime.AddCleanup(dbs, func(readers [2]*geoip2.Reader) {
		for _, r := range readers {
			if r != nil {
				r.Close()
			}
		}
	}, [2]*geoip2.Reader{dbs.asn, dbs.city})
	return dbs, nil
}

// TODO: 定期更新するようにする
func FetchGeoLocation(dir *os.Root, tmpDirPath, fetchInfoPath string) (*GeoLocationInfo, error) {
	var oldFetchInfo []*FetchInfo
	if fetchInfoPath != "" {
		fetchInfoRaw, err := dir.ReadFile(fetchInfoPath)
		if err != nil {
...
SKIPPED:
	...
	*/
	...
	if err != nil {
		...
		err := dir.MkdirAll(tmpDirPath, 0700)
		if err != nil {
			return nil, fmt.Errorf("failed to create temporary directory: %w", err)
		}

		tmpPrefix := time.Now().Format("2006-01-02-150405") + "_"
		...
		for _, c := range fetchCandidates {
			...
			tmpFile, err := dir.Create(filepath.Join(tmpDirPath, tmpPrefix+c.Name))
			if err != nil {
			...
			c.SavedPath = filepath.Join(tmpDirPath, tmpPrefix+c.Name)
	...
	for _, c := range useCandidates {
		...
		data, err := dir.ReadFile(c.SavedPath)
		if err != nil {
			return nil, fmt.Errorf("failed to read GeoLite.mmdb file: %w", err)
		}
	...
	// Pass the readers as the cleanup arg rather than closing over dbs.
	// runtime.AddCleanup panics ("cleanup function closes over ptr,
	// cleanup will never run") if the cleanup func references the very
	// object it's attached to — that keeps dbs reachable forever and the
	// cleanup would never fire. The reader pointers are independent heap
	// objects, so capturing them is safe.
	runtime.AddCleanup(dbs, func(readers [2]*geoip2.Reader) {
		for _, r := range readers {
			if r != nil {
				r.Close()
	...
	if len(removeOldInfo) > 0 {
		...
		for _, old := range removeOldInfo {
			if old.SavedPath != "" {
				if err := dir.Remove(old.SavedPath); err != nil {
	...
	if fetchInfoPath != "" && (len(oldFetchInfo) == 0 || len(removeOldInfo) > 0) {
		...
		if err := dir.WriteFile(fetchInfoPath, fetchInfoRaw, 0644); err != nil {
