...
struct stat_counters { /* go:Add,String */
  uint64_t rx_hook_call_count; // HELP Number of times the XDP hook is called.
  uint64_t rx_byte_total; // HELP Total number of bytes received. (including not handled)
  ...
  uint64_t arp_packet_total; // HELP Number of ARP packets received.
  ...
  uint64_t tcp_no_dest_entry_total; // HELP Number of packets dropped due to no backend available.
  uint64_t tcp_syn_packet_total; // HELP Number of TCP SYN packets received.
  uint64_t tcp_synack_packet_total; // HELP Number of TCP SYN-ACK packets received.
  uint64_t tcp_rst_packet_total; // HELP Number of TCP RST packets received.
  uint64_t tcp_ack_packet_total; // HELP Number of TCP ACK packets received.
  uint64_t tcp_fin_packet_total; // HELP Number of TCP FIN packets received.
  uint64_t tcp_urg_packet_total; // HELP Number of TCP URG packets received.
  uint64_t tcp_psh_packet_total; // HELP Number of TCP PSH packets received.
  uint64_t tcp_ece_packet_total; // HELP Number of TCP ECE packets received.
  uint64_t tcp_cwr_packet_total; // HELP Number of TCP CWR packets received.
  uint64_t tcp_other_flag_packet_total; // HELP Number of TCP packets received with other flags.
  uint64_t tcp_no_flag_packet_total; // HELP Number of TCP packets received without any flags.
  uint64_t udp_packet_total; // HELP Number of UDP packets received.  
  ...
  uint64_t quiclb_version_negotiation_total; // HELP Number of QUIC-LB version negotiation packets received.
  uint64_t quiclb_version_1_total; // HELP Number of QUIC-LB version 1 packets received.
  uint64_t quiclb_version_2_total; // HELP Number of QUIC-LB version 2 packets received.
  uint64_t quiclb_other_version_total; // HELP Number of QUIC-LB packets received with other versions.
  uint64_t quiclb_initial_packet_total; // HELP Number of QUIC-LB initial packets received.
  uint64_t quiclb_zero_rtt_packet_total; // HELP Number of QUIC-LB 0-RTT packets received.
  uint64_t quiclb_handshake_packet_total; // HELP Number of QUIC-LB handshake packets received.
  uint64_t quiclb_retry_packet_total; // HELP Number of QUIC-LB retry packets received. (this may bug or attack)

  uint64_t buggy_dest_ip_zero_total; // HELP Number of times a destination entry with IP address 0 is encountered (indicating a bug).
  uint64_t icmp_echo_reply_total; // HELP Number of ICMPv4 echo replies sent for VIP-destined echo requests.
  uint64_t icmp_dropped_total; // HELP Number of VIP-destined ICMPv4 packets dropped (echo requests when reply mode is off, plus every non-echo ICMP to the VIP).
} ALIGN8;
// clang-format on
...
#define LB_CONFIG_FLAG_XDP_DISABLED (1 << 1)
// When set, VIP-destined ICMPv4 echo requests are answered in-XDP (echo reply,
// XDP_TX). When clear, they are dropped (the VIP is not kernel-assigned, so
// passing them up just wastes a round-trip for the same no-response).
#define LB_CONFIG_FLAG_ICMP_ECHO_REPLY (1 << 2)
...
__always_inline void expand(uint8_t out[16],uint8_t* input, uint8_t full_len,uint8_t pass) {
  ...
  */
  memcpy(out,input,half_len);
  /*
  for(int i = half_len; i < 16; i++) {
     VOLATILE(out)[i] = 0;
  }*/
  memset(&out[half_len],0,16 - half_len);
...
// see https://github.com/torvalds/linux/blob/d91a46d6805af41e7f2286e0fc22d498f45a682b/tools/testing/selftests/bpf/progs/crypto_common.h#L30
...
struct temporary_encryption_buffer {
    __uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
    __uint(max_entries, 1);
    __type(key, __u32);
    __type(value, struct tempbuffer_t);
} temporary_encryption_buffer_map SEC(".maps");
...
__always_inline int connection_id_decrypt(uint8_t* output, const uint8_t* connection_id_bytes,uint8_t input_len,bool short_server_id, const char* context,struct stat_counters* c) {
    ...
    debugk("%s: input_len=%d, short_server_id=%u", context, input_len, short_server_id);
    ...
    tempbuffer = bpf_map_lookup_elem(&temporary_encryption_buffer_map, &zero);
    if(!tempbuffer) {
        debugk("%s:BUG!! no temporary encryption buffer found", context);
        return -ENOENT;
    }
    uint8_t* temporary = tempbuffer->buf;
    int ret = bpf_dynptr_from_mem(tempbuffer->buf,sizeof(tempbuffer->buf), 0, &temporary_dynptr);
    if (ret < 0) {
...
struct stat_counters* c,
    ...
    if (!dest) {
      debugk("ASSERTION FAILURE: no dest entry for %u", server_id);
...
__always_inline struct destination_entry* handle_connection_id(struct quiclb_connection_id* conn_id,
    ...
    if(!cached_server_id) {
      ...
      debugk("%s conn_id server_id=%u", context, server_id);
      // update cache
...
int test_decrypt(struct xdp_md* ctx) {
  ...
  struct stat_counters c;
  uint8_t input_output[21];
  for(int i = 0; i < sizeof(data->connection_id); i++) {
...
// send_icmp_echo_reply turns a VIP-destined ICMPv4 echo request in place into an
// echo reply and leaves it for XDP_TX: swap the L2 addresses, swap the L3
// addresses (source becomes the VIP), flip the ICMP type 8->0. Same size in and
// out (payload untouched), so no length adjustment. The ICMP checksum is patched
// incrementally (RFC 1624) off the single 16-bit word that changed — the type/
// code word went from 0x0800 to 0x0000 — instead of re-summing the variable-
// length payload (which a bounded eBPF loop can't walk safely). Caller must have
// verified eth + iphdr + icmphdr are within bounds and protocol == ICMP.
__always_inline void send_icmp_echo_reply(struct xdp_md* md, const struct lb_config* config) {
    void* data = (void*)(uint64_t)md->data;
    void* data_end = (void*)(uint64_t)md->data_end;
    ...
    struct iphdr* ip = (struct iphdr*)(eth + 1);
    __be32 client = ip->saddr;
    ip->saddr = config->vip_address; // reply from the VIP
    ip->daddr = client;
    ip->ttl = 64;
    compute_ip_checksum(ip);

    struct icmphdr* icmp = (struct icmphdr*)(ip + 1);
    // type/code is one 16-bit word: request is 0x08 0x00, reply is 0x00 0x00.
    // RFC 1624 incremental update: HC' = ~(~HC + ~m + m'), with m=htons(0x0800),
    // m'=htons(0x0000). ~m' is 0xffff, which is the additive identity, so it drops.
    uint32_t sum = (uint16_t)~icmp->checksum;
    sum += (uint16_t)~htons(0x0800);
...
#define IP_KEY_FAMILY_IPV4 0 /* go: */
#define IP_KEY_FAMILY_IPV6 1 /* go: */

struct ip_key_t { /* go: */
    uint8_t family;          // 4 bytes 0: IPv4, 1: IPv6
    uint8_t protocol;        // protocol number (e.g., TCP=6, UDP=17)
    uint16_t dest_port;        // destination port in network byte order
    uint32_t pad;             // 4 bytes for future use
    uint8_t addr_prefix[8];  // address prefix (ipv4: first 24 bits, ipv6: first 64 bits)
} PACKED;
...
struct isn_lsb_distribution {
    __uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
    __uint(max_entries, 256);
    __type(key, uint32_t);
    __type(value, __u64);
} isn_lsb_distribution_map SEC(".maps");

__always_inline void count_isn_lsb(uint32_t isn) {
    uint32_t lsb = isn & 0xFF;
    __u64* counter = bpf_map_lookup_elem(&isn_lsb_distribution_map, &lsb);
    if(counter) {
        __sync_fetch_and_add(counter, 1);
    }
}

struct src_ip_counters {
    __uint(type, BPF_MAP_TYPE_LRU_PERCPU_HASH);
    __uint(max_entries, 1024); // * 12 cpu
    __type(key, struct ip_key_t);
    __type(value, __u64);
} src_ip_counters_map SEC(".maps");

struct packet_size_map {
  __uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
  __uint(max_entries, 1519); // max packet size
  __type(key, uint32_t);
  __type(value, __u64);
} packet_size_map SEC(".maps");

__always_inline void count_src_ip(uint8_t* src_ip, uint8_t ip_version,uint8_t protocol,uint16_t dest_port) {
    struct ip_key_t key = {};
    key.protocol = protocol;
    key.dest_port = dest_port;
    key.pad = 0;
    if(ip_version == 4) {
        key.family = IP_KEY_FAMILY_IPV4;
        key.addr_prefix[0] = src_ip[0];
        key.addr_prefix[1] = src_ip[1];
        key.addr_prefix[2] = src_ip[2];
        key.addr_prefix[3] = src_ip[3]; // currently, count full ipv4 address
        key.addr_prefix[4] = 0; // padding
        key.addr_prefix[5] = 0; // padding
        key.addr_prefix[6] = 0; // padding
        key.addr_prefix[7] = 0; // padding  
    } else if(ip_version == 6) {
        key.family = IP_KEY_FAMILY_IPV6;
        key.addr_prefix[0] = src_ip[0];
        key.addr_prefix[1] = src_ip[1];
        key.addr_prefix[2] = src_ip[2];
        key.addr_prefix[3] = src_ip[3];
        key.addr_prefix[4] = src_ip[4];
        key.addr_prefix[5] = src_ip[5];
        key.addr_prefix[6] = src_ip[6];
        key.addr_prefix[7] = src_ip[7];
    } else {
        return; // unsupported ip version
    }
    __u64* counter = bpf_map_lookup_elem(&src_ip_counters_map, &key);
    if(!counter) {
        // because map is per-cpu, we can safely create new entry
        __u64 initial_value = 1;
        int err = bpf_map_update_elem(&src_ip_counters_map, &key, &initial_value, BPF_NOEXIST);
        if(err < 0) {
            // failed to create new entry, 
            debugk("DEBUG: count_src_ip: bpf_map_update_elem failed with %d", err);
        }
    } else {
        __sync_fetch_and_add(counter, 1);
    }
...
#define PKT_INITIAL 0
#define PKT_HANDSHAKE 1
#define PKT_0RTT 2
#define PKT_RETRY 3

#define QUIC_LONG_PACKET_TYPE(t) ((t & 0x30) >> 4)

#define QUIC_VERSION_NEGOTIATION 0x00000000
#define QUIC_VERSION_1 0x00000001
#define QUIC_VERSION_2 0x6b3343cf

__always_inline void detect_quic_packet_type(struct quic_long_packet* pkt,struct stat_counters* c) {
  int pkt_type = -1;
  uint32_t version = (uint32_t)(pkt->version[0] << 24 |
                               pkt->version[1] << 16 |
                               pkt->version[2] << 8  |
                               pkt->version[3]); 
  switch(version)  {
    case QUIC_VERSION_NEGOTIATION:
      c->quiclb_version_negotiation_total++;
      break;
    case QUIC_VERSION_1: // version 1
      c->quiclb_version_1_total++;
      switch (QUIC_LONG_PACKET_TYPE(pkt->first_byte)) {
        case 0x00:
          pkt_type = PKT_INITIAL;
          break;
        case 0x01:
          pkt_type = PKT_0RTT;
          break;
        case 0x02:
          pkt_type = PKT_HANDSHAKE;
          break;
        case 0x03:
          pkt_type = PKT_RETRY;
          break;
        default:
          debugk("DEBUG: [BUG] detect_quic_packet_type: unknown long packet type %u",pkt->first_byte);
          break;
      }
      break;
    case QUIC_VERSION_2:
      // see https://datatracker.ietf.org/doc/html/rfc9369#name-long-header-packet-types
      c->quiclb_version_2_total++;
      switch(QUIC_LONG_PACKET_TYPE(pkt->first_byte)) {
        case 0x00:
          pkt_type = PKT_RETRY;
          break;
      ...
      break;
    default:
      c->quiclb_other_version_total++;
      break;
  }
  if(pkt_type == PKT_INITIAL) {
    c->quiclb_initial_packet_total++;
  }
  else if(pkt_type == PKT_HANDSHAKE) {
    c->quiclb_handshake_packet_total++;
  }
  else if(pkt_type == PKT_0RTT) {
    c->quiclb_zero_rtt_packet_total++;
  }
  else if(pkt_type == PKT_RETRY) {
    c->quiclb_retry_packet_total++;
  }
}
...
#define FLAG_IS(FLAG) ((tcp->th_flags & FLAG) != 0)
      bool flag_found = false;
      if(FLAG_IS(TH_SYN)) {
        if(FLAG_IS(TH_ACK)) {
          ++c->tcp_synack_packet_total;
        }
        else{
          ++c->tcp_syn_packet_total;
          count_isn_lsb(ntohl(tcp->th_seq));
        }
        flag_found = true;
      }
      if(FLAG_IS(TH_RST)) {
        ++c->tcp_rst_packet_total;
        flag_found = true;
      }
      if(FLAG_IS(TH_FIN)) {
        ++c->tcp_fin_packet_total;
        flag_found = true;
      }
      if(FLAG_IS(TH_URG)) {
        ++c->tcp_urg_packet_total;
        flag_found = true;
      }
      if(FLAG_IS(TH_PUSH)) {
        ++c->tcp_psh_packet_total;
        flag_found = true;
      }
      if(FLAG_IS(TH_ACK) && !FLAG_IS(TH_SYN)) { // SYN-ACKはsyn側でカウントしているため除外
        ++c->tcp_ack_packet_total;
        flag_found = true;
      }
      if(FLAG_IS(0x0040)) { /* ECE */
        ++c->tcp_ece_packet_total;
        flag_found = true;
      }
      if(FLAG_IS(0x0080)) { /* CWR */
        ++c->tcp_cwr_packet_total;
        flag_found = true;
      }
      if(!flag_found) {
        if(tcp->th_x2) { /* other flags */
          ++c->tcp_other_flag_packet_total;
        } else {
          ++c->tcp_no_flag_packet_total;
        }
      }
      ...
      *dest = search_consistent_hash_based_destination(config, hash_server_id(config, src_addr, ntohs(tcp->source), 0x55));
      ...
      if(is_long_header) {
        ...
        detect_quic_packet_type(quic, c);
...
__always_inline uint16_t get_dest_port(void* data,void* data_end, uint8_t protocol) {
   if ((void*)data + sizeof(struct transport_header) > data_end){
        return 0; // not enough data
   }
   if(protocol == IPPROTO_TCP || protocol == IPPROTO_UDP) { 
        struct transport_header* transport = (struct transport_header*)data;
        return transport->dest_port;
   }
   return 0;
}
...
int lb_main(struct xdp_md* ctx) {
  ...
  // record packet complete size
  uint32_t pkt_size = data_end - data;
  if(pkt_size <= 1518) { // avoid out of bound
    __u64* size_counter = bpf_map_lookup_elem(&packet_size_map, &pkt_size);
    if(size_counter) {
      __sync_fetch_and_add(size_counter, 1);
    }
  }
  ...
  c->rx_hook_call_count++;
  c->rx_byte_total += data_end - data;
  ...
  if(config->flags & LB_CONFIG_FLAG_XDP_DISABLED) {
    debugk("XDP is disabled via config");
    EXIT(XDP_PASS);
  }
  ...
  if(eth->h_proto == htons(ETH_P_IP)) {
      ...
      struct iphdr* ip = (struct iphdr*)(eth + 1);
      uint16_t dest_port = (ip->ihl != 5 || (ip->frag_off & ntohs(IP_OFFMASK)) != 0) ? 0 : get_dest_port(ip + 1, data_end, ip->protocol);
      count_src_ip((uint8_t*)&ip->saddr, 4, ip->protocol,dest_port);
      ...
      // VIP-destined ICMPv4 echo request: answer it here (echo-reply mode) or drop
      // it. The VIP is not assigned on this host, so passing it to the kernel would
      // just be a wasted round-trip to the same no-response. Handled before
      // handle_transport, which is TCP/UDP-only.
      if (ip->protocol == IPPROTO_ICMP) {
        ...
        struct icmphdr* icmp = (struct icmphdr*)(ip + 1);
        if (icmp->type == ICMP_ECHO && (config->flags & LB_CONFIG_FLAG_ICMP_ECHO_REPLY)) {
          send_icmp_echo_reply(ctx, config);
          ++c->icmp_echo_reply_total;
          EXIT(XDP_TX);
        }
        // Everything else VIP-destined ICMP (echo with reply mode off, or any
        // non-echo type) is dropped: the kernel has no VIP, so it cannot answer.
        ++c->icmp_dropped_total;
        EXIT(XDP_DROP);
      }
      ...
      uint16_t dest_port = get_dest_port(ipv6 + 1, data_end, ipv6->ip6_nxt);
      count_src_ip((uint8_t*)&ipv6->ip6_src, 6, ipv6->ip6_nxt, dest_port);
  ...
  // check for bug. see bug_2026_01_13.md for details
  if(dest->ip_address == 0) {
    debugk("ASSERTION FAILURE: dest ip address is 0");
    ++c->buggy_dest_ip_zero_total;
    EXIT(XDP_DROP);
  }
