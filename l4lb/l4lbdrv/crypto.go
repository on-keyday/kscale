...
func InitCrypto(logger *slog.Logger, binPath string, cryptoMap, cryptoMapDir string, sharedKey []byte) error {
	...
	logger.Info("Go binding type assertions for crypto passed")
	...
	logger.Info("Running crypto init program to set shared key")
	...
	if !cryptoMapExists {
		...
		logger.Info("Crypto context map pinned", slog.String("path", cryptoMap))
	} else {
		logger.Info("Crypto context map already exists, using existing map", slog.String("path", cryptoMap))
	}
