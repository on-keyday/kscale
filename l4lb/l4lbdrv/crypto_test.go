...
func TestCrypto(t *testing.T) {
	...
	err := l4lbdrv.InitCrypto(slog.Default(), "../c/init_crypto.o", "/sys/fs/bpf/__crypto_ctx_map", "/sys/fs/bpf/", key)
	if err != nil {
		t.Fatalf("Failed to initialize crypto: %v", err)
	}
	t.Log("Crypto initialized successfully")
	bound, err := l4lbdrv.BindBalancer(slog.Default(), "../c/lb.o", "", "/sys/fs/bpf/")
	if err != nil {
