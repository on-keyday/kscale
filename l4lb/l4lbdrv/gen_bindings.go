...
var reHelpComment = regexp.MustCompile(`\s*// HELP (.*)`)
...
func parseCSource(filename string) (*CFile, error) {
	...
	for scanner.Scan() {
		...
		case ParserStateStruct:
			...
			helpComment := reHelpComment.FindStringSubmatch(line)
			if len(helpComment) >= 2 {
				f.Help = helpComment[1]
			}
...
const jsonTemplate = `{
 ...
 {{- range $cfile := .CFiles }}
  {{- range $s := $cfile.CStructs }}
	{{- if ne $s.Name "stat_counters"}}
	  {{- continue }}
	{{- end }}
    {{- range $i, $f := $s.Fields }}
	{{- if $i}},{{end}}
	{
		"metric_name": "ksdk_l4lb_ebpf_{{ $f.Name }}",
		"help": "{{ $f.Help }}",
		"long_name": "L4lbEbpf{{ snakeCaseToCamelCase $f.Name }}",
		"value_type": "counter",
		"source_metric": "ebpf_stats",
		"hold_type": "{{ goType $f }}",
		"to_prometheus": "$dst = $src.{{ snakeCaseToCamelCase $f.Name }}",
		"labels": [
			{"name": "instance"}
		],
		"optional": true,
...
func emitMetricsBindingsGo(p *Parsed) error {
	const metricsTargetFilename = "../../access/defs/internal/stats/ebpf_metrics.json"
	tmpl := template.New(metricsTargetFilename)
	tmpl = tmpl.Funcs(template.FuncMap{
		"goType":               goType,
		"snakeCaseToCamelCase": snakeCaseToCamelCase,
	})

	if _, err := tmpl.Parse(jsonTemplate); err != nil {
		return err
	}
	...
	targetFilename := metricsTargetFilename
	if err := os.WriteFile(targetFilename, buf.Bytes(), 0644); err != nil {
		return err
	}
	return nil
}
...
func run() error {
	...
	if err := emitMetricsBindingsGo(&parsed); err != nil {
		return fmt.Errorf("Failed to emit metrics bindings: %v", err)
	}
