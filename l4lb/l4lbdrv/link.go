...
func AttachToLink(logger *slog.Logger, link netlink.Link, fd int) (*LinkAttacher, error) {
	if err := netlink.LinkSetXdpFd(link, fd); err != nil {
		return nil, fmt.Errorf("Failed to attach to link %s: %w", link.Attrs().Name, err)
	}
	logger.Info("Attached balancer XDP program", slog.Int("fd", fd), slog.String("interface", link.Attrs().Name))
	a := &LinkAttacher{
		attachedLink:    link,
		linkCheckTicker: time.NewTicker(LinkCheckFrequency),
		linkCheckDone:   make(chan struct{}),
		tickerStopped:   make(chan struct{}), // for safe shutdown
	}
	linkCheckDone := a.linkCheckDone
	ticker := a.linkCheckTicker
...
func (a *LinkAttacher) Close() error {
	...
	a.linkCheckTicker.Stop()
	<-a.tickerStopped // wait for goroutine to exit
